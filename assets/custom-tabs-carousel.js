  (function(){
    ...
    function getTabsScrollElement(btn){
      if(!btn) return tabsShell || null;

      /* Pick the real horizontal scrolling element. In this theme the visual
         clipping can happen from CSS on different wrappers, so relying only on
         tabsShell is not enough in every viewport/browser. */
      ...
      return tabsShell || null;
    }

    function ensureTabVisible(btn){
      var shell = getTabsScrollElement(btn);
      if(!shell || !btn) return;
      ...
      window.requestAnimationFrame(function(){
        ...
        var padding = Math.min(180, tabsShell.clientWidth);        
        var current = shell.scrollLeft || 0;
        var max     = Math.max(0, shell.scrollWidth - shell.clientWidth);

        var btnLeft  = (btnRect.left - shellRect.left) + current;
        var btnRight = btnLeft + btnRect.width;

        var visibleLeft  = current + padding;
        var visibleRight = current + shell.clientWidth - padding;

        var target = current;

        if(btnRight > visibleRight){
          target = btnRight - shell.clientWidth + padding;
        } else if(btnLeft < visibleLeft){
          target = btnLeft - padding;
        }
        ...
        window.setTimeout(function(){ setIndicator(btn); }, 120);
        window.setTimeout(function(){ setIndicator(btn); }, 360);
      });
    }
